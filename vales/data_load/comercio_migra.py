...
def precargar_relaciones():
    ...
    # Usado para evitar duplicados de CUIT entre lotes
    relaciones['used_cuits'] = set()
...
def procesar_lote(records, cache, batch_start):
    ...
    proveedores = []
    # usar el set global pasado en cache para evitar duplicados entre lotes
    used_cuits = cache.get('used_cuits', set())
    ...
    for idx, record in enumerate(records):
        try:
            ...
            # Servicios
            servicio_value = str(record.get('SERVICIO', '')).strip()[:5].upper()
            servicio_list = cache['servicios'].get(servicio_value, [])
            if servicio_list:
                id_servicio = servicio_list[0]
            else:
                id_servicio = cache['default_servicio']

            # Manejo de CUIT con restricción UNIQUE (sanitizar no numéricos)
            cuit_raw = str(record.get('CUIT', '')).strip()
            ...
            proveedor = Comercio(
                ...
                pago=pago_comercio,
                bonificacion_comercio=bonificacion,
                gasto_adm_comercio=gastos,
                observacion_comercio=nota
            )
...
def cargar_datos():
    ...
    try:
        ...
        dbf_path = os.path.join(BASE_DIR, 'data_load', 'datavfox', 'comercio.DBF')
        total_registros = 0
        ...
        with transaction.atomic():
            ...
            for i in range(0, total_registros_dbf, batch_size):
                ...
                if proveedores_batch:
                    try:
                        Comercio.objects.bulk_create(proveedores_batch)
